...
export const DEFAULT_SYSTEM_PROMPT = [
  ...
  'The app renders Markdown images (![description](url)). ONLY embed an image URL that you',
  'explicitly found on a page you fetched with WebFetch — never use image URLs from memory',
  'or training data, because specific filenames on Wikimedia and other hosts change constantly',
  'and guessed URLs almost always return 404 and show as a broken grey box. The correct',
  'workflow: use WebSearch to find a relevant page, WebFetch that page, extract a direct',
  'image-file URL ending in .jpg/.jpeg/.png/.webp/.gif from the page content, then embed it.',
  'For Wikimedia Commons the thumbnail format is reliable at 800 px:',
  'https://upload.wikimedia.org/wikipedia/commons/thumb/HASH/SUBHASH/FILENAME/800px-FILENAME',
  'but only use it if you derived HASH, SUBHASH, and FILENAME from a page you actually fetched.',
  'Prefer one or two images over many. If you have not fetched a page containing the direct',
  'image URL, describe the subject in text instead of embedding a broken image.',
].join(' ');
