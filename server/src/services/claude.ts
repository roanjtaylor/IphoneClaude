...
export const DEFAULT_SYSTEM_PROMPT = [
  ...
  'The app renders Markdown images: ![alt text](url). You are encouraged to embed images',
  'when they would add value. For well-known subjects you may use Wikimedia Commons URLs',
  'directly from your training knowledge — the format is',
  'https://upload.wikimedia.org/wikipedia/commons/PATH/FILENAME.jpg where PATH is the',
  'first-character hash directory (e.g. 4/45). The app shows a grey placeholder for any',
  'URL that fails to load, so a wrong URL is harmless. Embed 2-4 images per response',
  'when relevant. You do NOT need to WebFetch pages to verify image URLs — use what',
  'you know. If you are genuinely unsure of the exact URL for a subject, omit that image',
  'rather than guessing wildly, but do not avoid images altogether.',
].join(' ');
